...
if os.path.isdir(DATA_DIR):
    ...
    LATEST_STATE_FILE = os.path.join(DATA_DIR, 'latest_state.json')
else:
    _state_dir = application_path
...
TUNNEL_STATE_FILE = os.environ.get('TUNNEL_STATE_FILE', os.path.join(_state_dir, 'tunnels_state.json'))
...
def _scrape_server_traffic(server, sid, my_conns):
        ...
        for proto in ['awg', 'awg2', 'awg3', 'awg_legacy', 'xray', 'telemt', 'wireguard']:
            if proto in server.get('protocols', {}):
                ...
                for client in clients:
                    ...
                    latest_states[key] = _extract_client_state(sid, proto, client)
    ...
    return server_updates, latest_states
    ...
    while True:
        try:
            ...
            for sid, server in enumerate(data.get('servers', [])):
                ...
                if server_states:
                    latest_snapshot['by_server_protocol_client'].update(server_states)
...
async def api_get_connection_config(request: Request, server_id: int, req: ConnectionActionRequest):
    ...
    try:
        ...
        try:
            ...
            err_text = str(e)
            if req.protocol != 'wireguard' and 'private key not stored' in err_text.lower():
...
@app.post('/api/my/connections/{connection_id}/remove', tags=["Self-service"])
...
@app.post('/api/my/connections/{connection_id}/rename', tags=["Self-service"])
